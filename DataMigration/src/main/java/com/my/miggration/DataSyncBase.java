...
public abstract class DataSyncBase {
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= rsmd.getColumnCount(); i++) {
					if(rsmd.getColumnName(i).equals("HSTR_SEQNO") || 
					   rsmd.getColumnName(i).equals("TX_DTTM")	  ||
					   rsmd.getColumnName(i).equals("CSTNO")      ||
					   rsmd.getColumnName(i).equals("DATA_CTNT")
					  ){
	...
	protected synchronized String buildLogInsertSql(List<String> colomnNameList, List<Field> columnValueList){
